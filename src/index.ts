import {VertxHttpGatewayConnectorOptionsType} from "./type/Types";
...
export class VertxHttpGatewayConnector {
    ...
    public stop() {
        this._isClose = true;
        this._reconnectList.forEach((job) => clearTimeout(job));
    ...
    private connect(i: number, url: string) {
        ...
        ws.on('message', async (data: Buffer) => {
            ...
            if (messageChunk.chunkType === chunkTypeArray[1]) {
                ...
                req.on('close', () => {
                    cache.setItem(String(messageChunk.requestId), null, {ttl: 0});
